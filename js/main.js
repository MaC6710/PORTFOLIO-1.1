...
$(document).ready(function () {
    ...
    $menuItems.on('click', function(e){
        ...
        fullpageswiper.slideTo(SlideIdx); // 슬라이드 이동
        $menuPage.fadeOut(300); // 메뉴 숨기기
        $hamburger.fadeIn(300); // 햄버거 보이기
    });
});
...
function showImage(imageSrc) {
    // 이미지 top0부터 표시
    $('#full-image').off('load').on('load', function() {
        $('.panel-inner').scrollTop(0);
    });
    $('#full-image').attr('src', imageSrc);
    if ($('#full-image')[0].complete) {
        $('#full-image').trigger('load');
    }
